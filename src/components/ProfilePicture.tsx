   ...
   return (
      <div className={cn("relative size-10", className)}>
         <Avatar className={cn("bg-zinc-100", className)}>
            <AvatarImage src={user.image || `https://api.dicebear.com/9.x/adventurer-neutral/svg?seed=${user.name ?? "User"}`} alt={user.name ?? "User"} />
            <AvatarFallback className="bg-secondary-foreground/40 text-white font-bold">{(user.name ? user.name.charAt(0) : "U")}</AvatarFallback>
         </Avatar>
